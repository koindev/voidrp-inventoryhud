...
function closeInventory() {
    ...
    $.post("http://voidrp-inventoryhud/NUIFocusOff", JSON.stringify({}));
}
...
$(document).ready(function () {
    ...
    $('#use').click(function (event, ui) {
        if (dragging) {
            ...
            if (itemData.usable) {
                ...
                $.post("http://voidrp-inventoryhud/UseItem", JSON.stringify({
    ...
    $('#give').click(function (event, ui) {
        if (draggingItem != null && dragging) {
            ...
            if (itemData.canRemove) {
                $.post("http://voidrp-inventoryhud/GetNearPlayers", JSON.stringify({
                    item: itemData,
                    action: 'give'
                }));
    ...
    $("#seize").mouseenter(function () {
        ...
        $.post("http://voidrp-inventoryhud/SeizeCash", JSON.stringify({
    ...
    $("#steal").mouseenter(function () {
        ...
        $.post("http://voidrp-inventoryhud/StealCash", JSON.stringify({
            target: destinationOwner
        }));
    });
    ...
    $('#drop').click(function (event, ui) {
        if (dragging) {
            ...
            if (itemData.canRemove) {
                ...
                $.post("http://voidrp-inventoryhud/DropItem", JSON.stringify({
                    item: itemData,
                    qty: dropCount
                }));
                successAudio.play();
...
function AttemptDropInEmptySlot(origin, destination, moveQty) {
    ...
    if (result === -1) {
        ...
        if (moveQty === item.qty) {
            ...
            $.post("http://voidrp-inventoryhud/MoveToEmpty", JSON.stringify({
            ...
            $.post("http://voidrp-inventoryhud/EmptySplitStack", JSON.stringify({
                originOwner: origin.parent().data('invOwner'),
                originSlot: origin.data('slot'),
                originTier: origin.parent().data('invTier'),
    ...
    if (result === -1) {
        ...
        if (originItem.itemId === destinationItem.itemId && destinationItem.stackable) {
            ...
            if (moveQty != originItem.qty && destinationItem.qty + moveQty <= destinationItem.max) {
                ...
                $.post("http://voidrp-inventoryhud/SplitStack", JSON.stringify({
                    originOwner: origin.parent().data('invOwner'),
                    originTier: origin.parent().data('invTier'),
                ...
                if (destinationItem.qty === destinationItem.max) {
                    ...
                    $.post("http://voidrp-inventoryhud/SwapItems", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originItem: origin.find('.item').data('item'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                    ...
                    $.post("http://voidrp-inventoryhud/CombineStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
                    ...
                    $.post("http://voidrp-inventoryhud/TopoffStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originItem: origin.find('.item').data('item'),
                        originSlot: originItem.slot,
                        originTier: origin.parent().data('invTier'),
...
$('.popup-body').on('click', '.player', function () {
    ...
    if (action === "nearPlayersGive") {
        ...
        if (givingItem != null) {
            ...
            $.post("http://voidrp-inventoryhud/GiveItem", JSON.stringify({
                target: target,
                item: givingItem,
                count: count
            }), function (status) {
        ...
        $.post("http://voidrp-inventoryhud/GiveCash", JSON.stringify({
            target: target,
            item: givingItem,
            count: count
        }), function (status) {
